...
busBin :: [Int]->Int->Bool
busBin lis x = busBinaria lista 0 (lenght lista) x

busBinaria :: [Int]->Int->Int->Int->Bool
busBinaria lista ini fin bus | ini > fin = False
   													 | bus == lista!!med = True
														 | bus > lista!!med = busBinaria lista (med+1) fin bus
														 | otherwise = busBinaria lista ini (med-1) bus
														 where med=(ini+fin)/2
